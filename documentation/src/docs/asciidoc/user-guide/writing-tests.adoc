...
[source,java,indent=0]
----
include::{testDir}/example/exception/UncaughtExceptionHandlingTestDemo.java[tags=user_guide]
----
...
The `assertDoesNotThrow` assertion can be used when you want to verify that a particular piece of code does not throw any exceptions. It is specially useful for when you want to test if a specific part of the code does not throw any exception.
